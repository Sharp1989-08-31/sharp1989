...
# PCA
site <- as.numeric(sapply(strsplit(rownames(t(log(correctedData))),'GU.'),function(x)x[2]))
Site<-c(
  rep('Site_1',sum(site<17)),
  rep('Site_2', sum(site>=17))
)
pcaData <- as.data.frame(cbind(t(log(correctedData)),Site))
...
p<-autoplot(PCA2, data=pcaData,label=F, 
            shape = 16, size = 4,  colour = 'Site',
            loadings = TRUE, loadings.label = TRUE,
            loadings.colour = 'lightgrey',
            loadings.label.colour = 'grey50',
            frame.colour = 'Site', frame = TRUE,
            loadings.label.size = 4,xlim=c(-0.4,0.6),
            main='PCA of BLGU chemical residues') + 
...
p
write.csv(cbind(top5_pc1,top5_pc2),'top_PCA of BLGU chemical residues.csv')
...
# PCA
site <- as.numeric(sapply(strsplit(colnames(plsGeneData),'GU.'),function(x)x[2]))
Site<-c(
  rep('Site_1',sum(site<17)),
  rep('Site_2', sum(site>=17))
)
pcaData <- as.data.frame(cbind(t(plsGeneData),Site))
pcaData[,-ncol(pcaData)] <- data.frame(apply(pcaData[,-ncol(pcaData)], 
                                             2, function(x) as.numeric(as.character(x))))
...
p<-autoplot(PCA2, data=pcaData,label=F, 
            ...
            main='PCA of BLGU gene expressions') + 
  geom_text(aes(label = rownames(pcaData)), size = 4,
