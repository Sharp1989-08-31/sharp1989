...
review <- read.csv('chemical-BLGU.csv', row.names = 'Chemical')
#review <- review[,-c(10,25)]
review <- review[-which(rowSums(review==0)> (ncol(review)/2)),]
...
# Preliminary PCA to identify potential outliers ("MUD 6" identified as massive outlier)
library("FactoMineR")
res.pca <- PCA(singlePAH_Data)

p2<-pca(singlePAH_Data, center=F, scale=F) # data alread centered earlier, so centre and scale = FALSE
pS<-as.numeric(as.factor(colnames(singlePAH_Data)))
pCol<-rainbow(9)[pS]
plot(p2$calres$scores[,2]~p2$calres$scores[,1], pch=16, cex=2, col=pCol,
     xlim=c(-1000,100), ylim=c(-40,10))
text(p2$calres$scores[,2]~p2$calres$scores[,1], labels=colnames(singlePAH_Data), col=pCol, cex=0.5, pos=3)
...
for(i in order(plsMods[[bestMod]]$cvres$rmse[,3])[1:6]){
  plot(
