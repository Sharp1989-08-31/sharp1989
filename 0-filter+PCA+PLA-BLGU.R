...
# Data to use for PLS
finalChem<-log10(t(correctedData))
finalGene<-log2(t(plsGeneData[,-c(9, 23)]))
nGenes<-vector()
nComps<-vector()
xVar<-vector()
yVar<-vector()
...
for(i in hasVIPs){
  uniqueVIPgene[[i]]<-finalGene[,plsVIPscores[,i]>1]
  uniqueMods[[i]]<-pls(as.matrix(uniqueVIPgene[[i]]), as.matrix(finalChem[,i]), center=TRUE, scale=TRUE, cv=1)
}
...
write.csv(modRes2, 'PLS-BLGU.csv')
